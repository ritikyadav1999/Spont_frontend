...
export function ExperienceCard({ experience }: ExperienceCardProps) {
  const content = (
    <>
      <div className={`relative h-36 sm:h-44 md:h-56 overflow-hidden bg-gradient-to-br ${experience.theme}`}>
      ...
      <div className="p-4 sm:p-5">
        <h4 className="font-headline text-base sm:text-[1.2rem] font-bold leading-tight text-on-surface">{experience.title}</h4>
        <p className="mt-1 text-xs sm:text-[0.8rem] text-on-surface-variant">{experience.date}</p>
        <p className="mt-2 line-clamp-2 text-xs sm:text-[0.8rem] text-on-surface-variant">{experience.location}</p>
        <span className="mt-3 sm:mt-4 inline-block text-[0.78rem] sm:text-[0.82rem] font-bold text-primary transition-transform group-hover:translate-x-0.5">
