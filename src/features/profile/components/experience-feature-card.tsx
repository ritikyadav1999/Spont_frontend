...
export function ExperienceFeatureCard({ experience }: ExperienceFeatureCardProps) {
  const content = (
    <div className="flex h-full flex-col md:flex-row">
      <div className={`relative min-h-40 sm:min-h-48 md:min-h-60 md:w-[44%] ${experience.theme}`}>
      ...
      <div className="flex flex-1 flex-col justify-center p-5 sm:p-7">
        <h4 className="max-w-[15rem] font-headline text-xl sm:text-[2rem] font-bold leading-[0.95] tracking-tight text-on-surface">
          {experience.title}
        </h4>
        <div className="mt-4 sm:mt-8">
          <p className="mb-3 text-[0.78rem] text-on-surface-variant">{experience.date}</p>
          <p className="mb-5 max-w-[21rem] text-xs sm:text-[0.92rem] leading-relaxed text-on-surface-variant">{experience.location}</p>
          <span className="text-xs sm:text-[0.9rem] font-bold text-primary transition-transform group-hover:translate-x-0.5">
